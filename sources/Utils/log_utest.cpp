...
bool Utils::uTestLogs( UnitTest *utest_p, std::string &name)
{
    ...
    LOG( LOG_UTEST_PARENT, "Shouldn't be seen 1 %d", message_num++);
    LOGS( LOG_UTEST_PARENT, "Shouldn't be seen" << " ever 1 " << message_num++);
    LOG( LOG_UTEST, "Shouldn't be seen %d", message_num++);

    // Test printing
    log()->enable( LOG_UTEST_PARENT);
    LOG( LOG_UTEST_PARENT, "Log message %d", message_num++);
    LOG_INC_INDENT( LOG_UTEST_PARENT);

    LOGS( LOG_UTEST_PARENT, "Log message " << "with stream " << message_num++);
    ...
    LOGVS( LOG_UTEST_PARENT, 4, "Fairly verbose message(" << 4 << ") with stream interface " << message_num++);
    LOG_DEC_INDENT( LOG_UTEST_PARENT);

    // These messages are too verbose for currently set level (5) we should not see them in the log file
    LOGV( LOG_UTEST_PARENT, 9, "Very verbose message(%d) with formatted print %d", 9, message_num++);
    LOGVS( LOG_UTEST_PARENT, 9, "Very verbose message(" << 9 << ") with stream interface " << message_num++);

    log()->disable( LOG_UTEST_PARENT);
    LOG( LOG_UTEST_PARENT, "Shouldn't be seen 2 %d", message_num++);
    LOGS( LOG_UTEST_PARENT, "Shouldn't be seen" << " ever 2 "  << message_num++);
